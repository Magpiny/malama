...
auto RunCobaltTask(OllamaClient *client_ptr, std::string model_name, std::string prompt_text,
    ...
    static_cast<void>(co_await client_ptr->ExecuteStreamTask(
        model_name, prompt_text, images_payload,
...
auto StreamWorker::InitializeGeneration(
    ...
    if (m_client_ptr) {
        boost::cobalt::spawn(
            ...
            RunCobaltTask(m_client_ptr.get(), std::string(model_name), std::string(prompt_text),
