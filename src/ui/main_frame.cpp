...
MainFrame::MainFrame(
    ...
    if (session_dir_error) {
        wxLogError("Failed to create sessions dir: %s", session_dir_error.message().c_str());
    }
    ...
    // Automatically query and render the historical chat state upon launch
...
void MainFrame::LoadMostRecentSessionOnStartup() noexcept {
    ...
    // Sort to locate matching active targets (Pinned items first, then descending recency)
    std::sort(
        sessions_list.begin(), 
        sessions_list.end(), 
...
void MainFrame::setup_menu_bar() noexcept {
    wxMenuBar *menu_bar_ptr = new (std::nothrow) wxMenuBar();
    if (menu_bar_ptr == nullptr) { return; }

    auto *file_menu_ptr = new (std::nothrow) wxMenu();
    if (file_menu_ptr == nullptr) { delete menu_bar_ptr; return; }
    
    file_menu_ptr->Append(
        id_menu_preferences, "Settings...\tCtrl+,", "Configure malama settings"
    );
    file_menu_ptr->AppendSeparator();
    file_menu_ptr->Append(
        id_menu_exit, "Exit\tAlt-X", "Terminate application framework"
    );
    menu_bar_ptr->Append(file_menu_ptr, "&File");

    auto *help_menu_ptr = new (std::nothrow) wxMenu();
    if (help_menu_ptr == nullptr) { delete menu_bar_ptr; return; }
    help_menu_ptr->Append(
        id_menu_licence, "Licence", "Display open-source licensing constraints"
    );
    help_menu_ptr->Append(
        id_menu_about, "About...", "Display platform implementation details"
    );
...
void MainFrame::setup_workspace_layout() noexcept {
    ...
    auto *sidebar = new (std::nothrow) SidebarPanel(
        m_splitter_window_ptr, m_history_manager_ptr.get()
    );
    ...
    m_splitter_window_ptr->SplitVertically(
        m_sidebar_panel_ptr, m_chat_panel_ptr, constants::default_sash_position
    );
}
