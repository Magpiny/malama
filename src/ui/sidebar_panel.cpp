...
void SidebarPanel::setup_layout() noexcept {
    ...
    m_new_chat_btn_ptr = new (std::nothrow) wxButton(
        this, id_control_new_btn, "+ New Chat", wxDefaultPosition, wxDefaultSize, wxBU_EXACTFIT);
...
void SidebarPanel::populate_sidebar() noexcept {
    ...
    std::sort(m_active_metadata.begin(), m_active_metadata.end(),
...
void SidebarPanel::on_context_menu(wxContextMenuEvent &WXUNUSED(event)) noexcept {
    ...
    context_menu.Bind(
        wxEVT_MENU,
        [this, selection_index](wxCommandEvent &) {
            m_history_manager_ptr->ToggleSessionPin(
